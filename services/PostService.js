...
const {
  convertToReadingPossibility,
} = require("../utilities/post/adjustungPostData");
...
const addPost = async (reqBody) => {
  try {
    ...
    const postExist = await PostRepo.getExistPost(
      userId,
      sub_category,
      date_from,
      date_to,
    ...
    const alerts = await newPostAlertService.getMatchedAlerts(
      sub_category,
      date_from,
      date_to,
      time_from,
    ...
    if (alerts && alerts.length > 0) {
      await newPostAlertService.handleSendAlerts(alerts, answer.id);
    }
    return {id: answer.id};
  } catch (err) {
    console.log(err);
